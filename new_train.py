...
from src.asdf_dataset import ASDFDataset
from src.models import ARONetModel, ASDFNetModel
...
def train(args):
    ...
    if args.name_dataset in ['abc','shapenet']:
        asdf_dataset = ASDFDataset(split='train', args=args)
        # print(asdf_dataset[0])
        train_loader = DataLoader(asdf_dataset, shuffle=True, batch_size=args.n_bs, num_workers=args.n_wk, drop_last=True)
        # check ARONetDataset output
        val_loader = DataLoader(ASDFDataset(split='val', args=args), shuffle=False, batch_size=args.n_bs, num_workers=args.n_wk, drop_last=True)
    ...
    model = ASDFNetModel(n_anc=n_anc, n_qry=args.n_qry, n_local=args.n_local, cone_angle_th=args.cone_angle_th, tfm_pos_enc=args.tfm_pos_enc, 
                        cond_pn=args.cond_pn, use_dist_hit=args.use_dist_hit, pn_use_bn=args.pn_use_bn, pred_type=args.pred_type, norm_coord=args.norm_coord)
    ...
    for i in range(epoch_latest, args.n_epochs):
        ...
        if n_epoch % args.freq_ckpt == 0:
            ...
            if args.multi_gpu:
                torch.save({'model':model.module.state_dict(), 'opt':opt.state_dict(), 'n_epoch':n_epoch, 'n_iter':n_iter}, f'{dir_ckpt}/{n_epoch}_{n_iter}_{start_time}.ckpt')
                           # {avg_loss_pred:.4}_{avg_acc:.4}.ckpt')
            else:
                torch.save({'model':model.state_dict(), 'opt':opt.state_dict(), 'n_epoch':n_epoch, 'n_iter':n_iter}, f'{dir_ckpt}/{n_epoch}_{n_iter}_{start_time}.ckpt') # {avg_loss_pred:.4}_{avg_acc:.4}.ckpt')
