...
from aro_net.Metric.iou import eval_iou
from aro_net.Metric.chamfer import eval_chamfer
from aro_net.Metric.hausdoff import eval_hausdoff


def cal_metrics(
    path_ref, path_rec, shape_id, dir_metrics, path_qry, path_occ, name_dataset
):
    try:
        _, pts_ref = sample(path_ref)
        ...
        dist_hsdf_rec2ref, dist_hsdf_ref2rec, dist_hsdf_max = eval_hausdoff(
            pts_rec, pts_ref
        )
        ...
        fout.write(str(iou) + " ")
        fout.write(
            str(dist_hsdf_rec2ref)
            + " "
            + str(dist_hsdf_ref2rec)
            + " "
            + str(dist_hsdf_max)
        )
...
def sample(mesh_file, num_samples=10000):
    mesh = trimesh.load(
        mesh_file, force="mesh", skip_materials=True, maintain_order=True, process=False
    )
    samples, _ = trimesh.sample.sample_surface_even(mesh, num_samples)
    return mesh, samples
    ...
    for category in categories:
        ...
        for shape_id in id_shapes:
            if is_ref:
                ...
                path_qrys.append(
                    f"{dir_dataset}/02_qry_pts_imnet/{category}/{shape_id}.npy"
                )
                if args.name_dataset == "shapenet":
                    path_occs.append(
                        f"{dir_dataset}/03_qry_occs_imnet/{category}/{shape_id}.npy"
                    )
                else:
                    path_occs.append(
                        f"{dir_dataset}/03_qry_dists/{category}/{shape_id}.npy"
                    )
            else:
                dir_mesh = os.path.join(
                    "experiments", args.name_exp, "results", args.name_dataset
                )
                path_shapes.append(os.path.join(dir_mesh, shape_id + "." + fext_mesh))
...
def eval(args):
    dir_metrics = os.path.join(
        "./experiments", args.name_exp, "metrics", args.name_dataset
    )
    ...
    tasks = []
    for idx, _ in enumerate(id_shapes):
        tasks.append(
            [
                path_shapes_ref[idx],
                path_shapes_rec[idx],
...
def report(args):
    ...
    _, id_shapes, _, _ = get_data_paths(args, is_ref=False)
    ...
    for shape_id in id_shapes:
        try:
            ...
            print(
                "[Warning] Fail to load:", os.path.join(dir_metrics, shape_id + ".txt")
            )
