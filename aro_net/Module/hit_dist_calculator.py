...
from aro_net.Method.mesh import load_mesh, create_mesh_o3d
from aro_net.Method.raycast import create_raycast_scene, cast_rays
...
class HitDistCalculator:
    def __init__(self, args) -> None:
        ...
        for split in {"train", "val", "test"}:
            for category in categories:
                os.makedirs(
                    f"{self.dir_dataset}/05_hit_dist/{category}/", exist_ok=True
                )
                id_shapes = (
    ...
    def cal_hit_distance(self, category, shape_id):
        ...
        for i in range(self.n_anc):
            origin = self.anc[i]
            ray_anc2pts_ = np.concatenate(
                [origin + np.zeros_like(norm_anc2pts[:, i, :]), norm_anc2pts[:, i, :]],
                axis=-1,
            )
    ...
    def cal_multi_processes(self):
        with Parallel(n_jobs=8) as p:
            p(
                delayed(self.cal_hit_distance)(category, shape_id)
                for category, shape_id in self.files
            )
