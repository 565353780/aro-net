...
class ASDFDataset(Dataset):
    def __init__(self,split, args) -> None:
        ...
        for category in categories:
            if self.split == 'train':
                id_shapes = open(f'{self.dir_dataset}/04_splits/{category}/asdf.lst').read().split()
                for shape_id in id_shapes:
                    self.files.append((category, shape_id))
            else:
                id_shapes = open(f'{self.dir_dataset}/04_splits/{category}/asdf_test.lst').read().split()
                for shape_id in id_shapes:
                    self.files.append((category, shape_id))
    ...
    def __getitem__(self, index):
        ...
        ftrs = np.load(f'{self.dir_dataset}/06_asdf_params_right/{category}/{shape_id}.npy')
