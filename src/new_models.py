...
class ASDFNetModel(nn.Module):
    def __init__(self, n_anc, n_qry, n_local, cone_angle_th, tfm_pos_enc=True, cond_pn=True, use_dist_hit=False, pn_use_bn=True, pred_type='occ', norm_coord=False):
        ...
        self.att_layer = nn.TransformerEncoderLayer(d_model=self.hidden_dim, nhead=8, batch_first=True)
        ...
        if self.pred_type == 'occ':
            ...
            self.fc_out = nn.Sequential(
                nn.Conv1d(self.n_anc*self.hidden_dim, self.n_anc*self.hidden_dim//2, 1),
                nn.Tanh(),
