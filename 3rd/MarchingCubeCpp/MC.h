...
static inline mcVec3f mc_internalCross(const mcVec3f &v1, const mcVec3f &v2) {
  return mcVec3f({{{v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
                    v1.x * v2.y - v1.y * v2.x}}});
}
...
void marching_cube(MC_FLOAT *field, muint nx, muint ny, muint nz,
                   mcMesh &outputMesh);
...
static inline muint mc_internalToIndex1D(muint i, muint j, muint k,
                                         const mcVec3i &size) {
  return (k * size.y + j) * size.x + i;
}

static inline muint mc_internalToIndex1DSlab(muint i, muint j, muint k,
                                             const mcVec3i &size) {
...
/*!
\brief Approximates the vertex position of the mesh from the scalar values along
an edge (va, vb). \param slab_inds slab indices global array \param mesh the
mesh \param va, vb edges values \param axis axis index 0/1/2 \param x, y, z
current slab index \param size slab indices array size
*/
static void mc_internalComputeEdge(mcVec3i *slab_inds, mcMesh &mesh,
                                   MC_FLOAT va, MC_FLOAT vb, int axis, muint x,
                                   muint y, muint z, const mcVec3i &size) {
  if ((va < 0.0) == (vb < 0.0))
    return;
  mcVec3f v = {{{MC_FLOAT(x), MC_FLOAT(y), MC_FLOAT(z)}}};
  v[axis] += va / (va - vb);
  slab_inds[mc_internalToIndex1DSlab(x, y, z, size)][axis] =
      muint(mesh.vertices.size());
  mesh.vertices.push_back(v);
  mesh.normals.push_back(mcVec3f({{{0, 0, 0}}}));
}

/*!
\brief Computes and acumulates the geometric normal of triangle formed by
vertices (a, b, c). \param mesh the mesh \param a, b, c vertex indices
*/
static inline void mc_internalAccumulateNormal(mcMesh &mesh, muint a, muint b,
                                               muint c) {
...
*/
inline void setDefaultArraySizes(muint vertSize, muint normSize,
                                 muint triSize) {
...
/*!
\brief Computes the mesh representing the zero isosurface of a 3D scalarfield
and outputs it to an indexed mesh. \param field scalarfield of real values in
C-contiguous order: the value for grid point (ix, iy, iz) is stored at index
(iz*ny + iy)*nx + ix \param nx, ny, nz grid dimension \param outputMesh indexed
mesh returned.
*/
void marching_cube(MC_FLOAT *field, muint nx, muint ny, muint nz,
                   mcMesh &outputMesh) {
  ...
  for (muint z = 0; z < nz - 1; z++) {
    for (muint y = 0; y < ny - 1; y++) {
      for (muint x = 0; x < nx - 1; x++) {
        ...
        const int config_n = ((vs[0] < 0) << 0) | ((vs[1] < 0) << 1) |
                             ((vs[2] < 0) << 2) | ((vs[3] < 0) << 3) |
                             ((vs[4] < 0) << 4) | ((vs[5] < 0) << 5) |
                             ((vs[6] < 0) << 6) | ((vs[7] < 0) << 7);
        ...
        if (y == 0 && z == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[0], vs[1], 0, x, y,
                                 z, size);
        if (z == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[2], vs[3], 0, x,
                                 y + 1, z, size);
        if (y == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[4], vs[5], 0, x, y,
                                 z + 1, size);
        mc_internalComputeEdge(slab_inds, outputMesh, vs[6], vs[7], 0, x, y + 1,
                               z + 1, size);

        if (x == 0 && z == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[0], vs[2], 1, x, y,
                                 z, size);
        if (z == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[1], vs[3], 1, x + 1,
                                 y, z, size);
        if (x == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[4], vs[6], 1, x, y,
                                 z + 1, size);
        mc_internalComputeEdge(slab_inds, outputMesh, vs[5], vs[7], 1, x + 1, y,
                               z + 1, size);

        if (x == 0 && y == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[0], vs[4], 2, x, y,
                                 z, size);
        if (y == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[1], vs[5], 2, x + 1,
                                 y, z, size);
        if (x == 0)
          mc_internalComputeEdge(slab_inds, outputMesh, vs[2], vs[6], 2, x,
                                 y + 1, z, size);
        mc_internalComputeEdge(slab_inds, outputMesh, vs[3], vs[7], 2, x + 1,
                               y + 1, z, size);

        edge_indices[0] = slab_inds[mc_internalToIndex1DSlab(x, y, z, size)].x;
        edge_indices[1] =
            slab_inds[mc_internalToIndex1DSlab(x, y + 1, z, size)].x;
        edge_indices[2] =
            slab_inds[mc_internalToIndex1DSlab(x, y, z + 1, size)].x;
        edge_indices[3] =
            slab_inds[mc_internalToIndex1DSlab(x, y + 1, z + 1, size)].x;
        edge_indices[4] = slab_inds[mc_internalToIndex1DSlab(x, y, z, size)].y;
        edge_indices[5] =
            slab_inds[mc_internalToIndex1DSlab(x + 1, y, z, size)].y;
        edge_indices[6] =
            slab_inds[mc_internalToIndex1DSlab(x, y, z + 1, size)].y;
        edge_indices[7] =
            slab_inds[mc_internalToIndex1DSlab(x + 1, y, z + 1, size)].y;
        edge_indices[8] = slab_inds[mc_internalToIndex1DSlab(x, y, z, size)].z;
        edge_indices[9] =
            slab_inds[mc_internalToIndex1DSlab(x + 1, y, z, size)].z;
        edge_indices[10] =
            slab_inds[mc_internalToIndex1DSlab(x, y + 1, z, size)].z;
        edge_indices[11] =
            slab_inds[mc_internalToIndex1DSlab(x + 1, y + 1, z, size)].z;
        ...
        for (size_t i = 0; i < n_triangles; i++) {
          mc_internalAccumulateNormal(
              outputMesh, outputMesh.indices[indexBase + i * 3 + 0],
