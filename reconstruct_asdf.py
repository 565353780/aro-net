...
from asdf_preprocess import get_anchor_feature
...
class Generator3D(object):
    ...
    def __init__(self, model, points_batch_size=100000,
                 ...
                 resolution0=64, upsampling_steps=2, chunk_size=512,
    ...
    def eval_points(self, data):
        ...
        q_ftrs = torch.tensor(np.expand_dims(q_ftrs, axis=0)).to(device=torch.device('cuda')).to(torch.float32)
        ...
        for idx in range(n_chunk):
            data_chunk = {}
            for key in data:
                if key == 'ftrs': continue
                if key == 'qry':
                    if idx < n_chunk - 1:
                        data_chunk[key] = data[key][:, chunk_size*idx:chunk_size*(idx+1), ...]
                        data_chunk['ftrs'] = q_ftrs[:, chunk_size*idx:chunk_size*(idx+1), ...]
                    else:
...
if __name__ == '__main__':
    ...
    dataset = ASDFDataset(split='asdf_test', args=args) # FIXME
    ...
    if args.name_dataset == 'shapenet':
        ...
        for category in categories:
            print(category)
            id_shapes_ = open(f'{dir_dataset}/04_splits/{category}/asdf_test.lst').read().split('\n')
